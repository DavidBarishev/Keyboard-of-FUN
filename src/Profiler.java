...
public class Profiler {
	 ...
	 * ///////////////////////////////// Barr's code
	 * 
	 * public profile compare(profile[] users, profile comparable, int
	 * threshold) { float[] proximity = new float[users.length]; for (int i = 0;
	 * i < users.length; i++) { proximity[i] = approximate(users[i],
	 * comparable); } int bestOption = bestOption(proximity, threshold); if
	 * (bestOption == -1) { return null; } else { proximityPercent = 100
	 * -((proximity[bestOption]/avgProximity(comparable.letters))*100); return
	 * users[bestOption]; } }
	 * 
	 * private static float avgProximity(letter[] letters) { int[] lettersTime =
	 * new int[23]; for(int i = 0;i < 23;i++){
	 * lettersTime[i]=letters[i].avgPressTime; } return
	 * avgProximity(lettersTime); }
	 * 
	 * private static int bestOption(float[] proximity, int threshold) { float
	 * best = -1; int bestProximity = -1; for (int i = 0; i < proximity.length;
	 * i++) { if (proximity[i] <= threshold && proximity[i] > best) { best =
	 * proximity[i]; bestProximity = i; } } return bestProximity; }
	 * 
	 * private static float approximate(profile user, profile comparable) {
	 * int[] letterProximity = new int[24]; for (int i = 0; i < 23; i++) {
	 * letterProximity[i] = Math.abs(user.letters[i].avgPressTime -
	 * comparable.letters[i].avgPressTime); } return
	 * avgProximity(letterProximity); }
	 * 
	 * private static float avgProximity(int[] letterProximity) { int sum = 0;
	 * for (int i = 0; i < letterProximity.length; i++) { sum +=
	 * letterProximity[i]; } return sum / letterProximity.length; }
	 */
	// Barr's code
	// ////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	/*
	 ...
	 * j++){ if(i < j){ min = i; } } } return min; }
	 * 
	 * //gives us the sum of a given array. public static double
	 * lastSum(double[] a){
	 * 
	 * double sum = 0;
	 * 
	...
	private static int percentageForProfile(profile user, profile optionUser) {
		...
		for (int i = 0; i < user.letters.length; i++) {
			averagePercentange += ((Math.abs(optionUser.letters[i].avgPressTime
